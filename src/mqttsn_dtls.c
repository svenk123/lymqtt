...
#if !defined(MBEDTLS_GCM_C) || !defined(MBEDTLS_KEY_EXCHANGE_PSK_ENABLED) ||   \
    !defined(MBEDTLS_SHA256_C)
#error                                                                         \
    "mbedTLS ohne GCM/PSK/SHA256 gebaut – TLS_PSK_WITH_AES_128_GCM_SHA256 nicht verfügbar."
#endif
...
static int udp_send(void *ctx, const unsigned char *buf, size_t len) {
  ...
  ssize_t r =
      sendto(n->sockfd, buf, len, 0, (struct sockaddr *)&n->peer, n->peerlen);
  if (r < 0)
...
static int udp_recv(void *ctx, unsigned char *buf, size_t len) {
  ...
  for (;;) {
    ...
    if (rv < 0) {
      if (errno == EINTR)
        /* Signal received - return WANT_READ so mbedtls_ssl_read returns
         * and allows g_stop check in main loop */
...
static void log_sock_local(int fd) {
  ...
  if (ss.ss_family == AF_INET) {
    ...
    ok = inet_ntop(AF_INET6, &sa6->sin6_addr, ip, sizeof(ip)) !=
         NULL; // <- sin6_addr!
  }
...
static int resolve_and_bind(net_dtls_t *n, const char *host, int port) {
  ...
  for (ai = res; ai; ai = ai->ai_next) {
    ...
    if (n->bind_iface &&
        net_bind_to_interface(sockfd, n->bind_iface, ai->ai_family) != 0) {
      close(sockfd);
      sockfd = -1;
...
static void mbedtls_dbg_cb(void *ctx, int level, const char *file, int line,
                           const char *str) {
  (void)ctx;
  /* Level 1..4, we give everything through – Filter makes
...
int net_dtls_init(net_dtls_t *n, const char *host, int port,
  ...
  log_info("net_dtls_init: requested iface=%s",
           bind_iface ? bind_iface : "(none)");
  ...
  int ret = mbedtls_ctr_drbg_seed(ctr, mbedtls_entropy_func, ent,
  ...
  if (psk_identity && psk_key && psk_len > 0 && (!ca && !cert && !key)) {
...
#else
    log_err("mbedTLS without PSK support built – enable e.g. "
    ...
    if (cert && key) {
      ...
      if ((ret = mbedtls_pk_parse_keyfile(&pkey, key, "", mbedtls_ctr_drbg_random, ctr)) != 0) {
    ...
    log_info("DTLS without PSK/X.509 – this will usually fail (Broker requires "
             "Auth).");
  }
  ...
  /* 1) SNI */
  const char *sni_eff = (sni_name && *sni_name)
                            ? sni_name
                            : (host && !is_ip_literal(host) ? host : NULL);

#if defined(MBEDTLS_SSL_SERVER_NAME_INDICATION)
  if (sni_eff) {
    int sret = mbedtls_ssl_set_hostname(
        ssl, sni_eff); /* IMPORTANT: on 'ssl', not n->ssl */
    if (sret == 0) {
      if (g_log_level >= 1)
        log_info("SNI enabled: %s", sni_eff);
    } else {
      char err[128];
      mbedtls_strerror(sret, err, sizeof err);
...
/* Optional: Ciphersuite-Liste restrict (only if Gateway wants GCM) */
  ...
  /* Success: output the negotiated suite & version (when verbose) */
  {
    ...
    log_info("DTLS connected: %s, vers=%s", mbedtls_ssl_get_ciphersuite(ssl),
...
int net_dtls_send(net_dtls_t *n, const uint8_t *buf, size_t len) {
  ...
  int ret;
  uint64_t t0 = now_ms();
  while ((ret = mbedtls_ssl_write((mbedtls_ssl_context *)n->ssl, buf, len)) <=
         0) {
    if (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
      if (timed_out(t0, n->timeout_ms))
        return 0;
...
int net_dtls_recv(net_dtls_t *n, uint8_t *buf, size_t buflen, int wait_ms) {
  if (!n->use_dtls) {
    ...
    if (rc < 0) {
      if (errno == EINTR)
        return 0; /* Signal received, treat as timeout */
      return -1;
    }
    ...
    if (r < 0) {
      /* Error */
      if (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK) return 0; /* treat as timeout */
  ...
  mbedtls_ssl_conf_read_timeout((mbedtls_ssl_config *)n->conf,
                                (uint32_t)wait_ms);
  uint64_t t0 = now_ms();
  int ret;
  while ((ret = mbedtls_ssl_read((mbedtls_ssl_context *)n->ssl, buf, buflen)) <=
         0) {
    ...
    if (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
      /* Check timeout */
      if (timed_out(t0, wait_ms))
        return 0; /* Timeout */
      /* Return 0 (timeout) to allow main loop to check g_stop.
       * This ensures signals are handled promptly. */
      return 0;
    }
